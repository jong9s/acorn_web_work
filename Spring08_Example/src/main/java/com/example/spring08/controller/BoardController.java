...
public class BoardController {
	...
	public String boardView(BoardDto requestDto, Model model) {
		 ...
		 *  search (검색조건), keyword (검색어) 가 들어있을 수도 있다.
		 *  검색어가 없는 경우 search 와 keyword 에는 null 이 들어있다.
		 */
	...
	public String list(Model model, 
			@RequestParam(defaultValue = "1") int pageNum, BoardDto dto) {
		// BoardDto 객체에는 keyword 와 search 가 있을 수도 있다. (없으면 null)
		
		// 응답에 필요한 데이터를 얻어내서
		BoardListResponse listResponse = service.getBoardList(pageNum, dto);
		// 모델 객체에 담고
		model.addAttribute("listRes", listResponse);
