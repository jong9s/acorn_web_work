...
public class MemberController {
	...
	public String delete(int num) {
		 ...
		 *  문자열을 자동추출해서 Integer.parseInt() 를 수행해서 실제 int 값으로 바꾼 다음
		 *  해당 값을 매개 변수에 전달해 준다.
	...
	public String insert(MemberDto dto) {
		 ...
		 *  단 MemberDto 클래스의 필드명과 폼 전송되는 파라미터명이 같아야 한다.
		 *  private String name <=> <input type="text" name="name">
		 *  private String addr <=> <input type="text" name="addr">
		 */
		service.addMember(dto);
		// 회원 목록보기로 "/member/list" 요청을 다시 하라는 redirect 응답하기
		// "redirect:리다이렉트 경로" 처럼 redirect: 로 시작하는 문자열을 리턴하면 된다.
