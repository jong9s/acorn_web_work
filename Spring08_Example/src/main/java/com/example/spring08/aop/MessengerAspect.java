...
public class MessengerAspect {
	...
	public Object checkReturn(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		// 리턴 값이 있는 메소드에 aspect 를 적용하면 반드시 해당 데이터를 리턴해야한다.
		
		// return obj;
		return "무슨 공부야? 놀자고"; // 다른 값을 리턴해줄 수도 있다.
	}
	
	
	// .. 은 매개변수의 모양을 상관하지 않겠다. (개수와 type 을 제한하지 않음)
	...
	public void checkGreeting(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		for(Object tmp : args) {
			// 찾고 싶은 type 을 확인한다
			if(tmp instanceof String) { // 만약 String type 이라면
				// 찾았다면 원래 type 으로 casting 한다
				String msg = (String)tmp;
				log.debug("매개 변수에 전달된 값: " + msg);
				if(msg.contains("해골")) {
					log.error("해골은 금지된 단어입니다. 메소드 호출을 차단합니다.");
					return; // 여기서 리턴하면 아래의 joinPoint.proceed(); 가 호출이 안된다.
				}
			}
			if(tmp instanceof MemberDto) { // 만약 MemberDto Type 이라면
