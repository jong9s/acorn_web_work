...
public class GalleryServiceImpl implements GalleryService{
	 ...
	 *  2. @Repository 어노테이션이 붙은 dao 에서 DB 관련 작업 중에 SQLException 이 발생하면
	 *     spring 이 해당 예외를 잡아서 DataAccessException 을 자동으로 발생시킨다(Transaction 에 영향을 주기 위해서)
	 *  3. @Mapper 를 이용해서 dao 를 만들면 해당 dao 에 자동으로 @Repository 어노테이션이 붙는다.
	 *  4. 서비스에서 어떤 동작을 하다가 에러가 난 경우 Transaction 에 영향을 주고 싶으면 일반 예외를 발생시키지 말고 
	 *     DataAccessException 을 throw 하면 Transaction 관리가 된다.
	 *  5. 커스텀 Exception 을 발생시켜서 Transaction 을 관리하고 싶으면 커스텀 Exception 클래스를 만들때
	 *     반드시 RuntimeException 클래스 말고 DataAccessException 클래스를 상속 받아서 만들고
	...
	public void createGallery(GalleryUploadRequest galleryRequest) {
		...
		for(int i = 0; i<images.length; i++) {
			...
			GalleryImageDto imageDto = GalleryImageDto.builder().galleryNum(num).saveFileName(saveFileName).build();
			// DB 에 저장한다
