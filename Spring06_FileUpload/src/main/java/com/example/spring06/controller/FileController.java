...
public class FileController {
	...
	/*
	 *  custom.properties 파일이 로딩되기 위해서는 main 메소드가
	 *  있는 클래스에 @PropertiesSource 어노테이션 설정이 되어있어야 한다.
	 *  
	 *  cusome.properties 파일에 있는 파일 업로드
	 *  경로를 읽어와서 필드에 담는다
	 */
	...
	public ResponseEntity<InputStreamResource> download(String orgFileName, String saveFileName, long fileSize) {
		/*
		 *  파일을 다우놀드 해주기 위해서는 원본 파일명, 저장된 파일명, 파일의 실제 크기가 필요하다
	...
	public String fileUpload2(Model model, FileDto dto) {
		...
		// 매개 변수에 전달된 FileDto 객체에 폼 전송된 내용이 모두 들어있다.
