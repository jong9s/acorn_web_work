...
<body>
	...
	<script>
		...
		const checkForm = ()=>{
			// 만약 아이디도 유효하고 비밀번호도 유효하고 이메일도 유효하다면
		...
		document.querySelector("#userName").addEventListener("input", (e)=>{
	        ...
	        // "영문자 소문자로 시작하고 5~10글자 이내로 입력" 이 검증 조건을 만족했는지 여부를 알아낸다
	        const reg = /^[a-z].{4,9}$/;
	        // 만약 정규 표현식을 통과하지 못했다면
	        if(!reg.test(inputId)) {
	        ...
	        // fetch() 함수를 이용해서 get 방식으로 입력한 아이디를 보내고 사용 가능 여부를 json 으로 응답받는다.
		...
		*/
        ...
        document.querySelector("#password2").addEventListener("input", checkPwd);
